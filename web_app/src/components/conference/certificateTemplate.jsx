...
const CertificateTemplate = ({ onTemplateSet ,certiDetails}) => {
  ...
  useEffect(() => {
    if (templateImageSrc) {
      ...
      img.onload = () => {
        ...
        if (onTemplateSet) {
          onTemplateSet({ ...selectedTemplate, image: img }); // Pass the template and image to the parent component
        }
      };
    }
  }, [templateImageSrc, selectedTemplate, onTemplateSet]);
  useEffect(() => {
  ...
  const getRectangles = () => {
    if (selectedTemplate.name === 'New Certificate Template') {
      return rectangles;
    }
    return selectedTemplate.rectangles || [];
  };
  ...
  const testTemplate = async () => {
   ...
   setShowTestImage(true);
    ...
    try {
      ...
      await Promise.all(
       selectedTemplate.name ==='New Certificate Template'? (titles.map(async (zone, i) => {
          if (zone === 'qrCode') {
            ...
            ctx.fillText( 'Educheck.tn',(rectangles[i].x1 + rectangles[i].x2) / 2,(rectangles[i].y1 + rectangles[i].y2) / 2 +20 );
          } else {
    ...
    <main>
			{!certiDetails &&<div  className="head-title">
              ...
              <span style={{color: '#FBBC05'}}>s</span>
					...
					<div>
					{!certiDetails &&<h3 style={{color:"#FBBC05"}}>Certificate Name</h3>}
					{certiDetails &&<h3 style={{color:"#FBBC05"}}>Select Certificate Template</h3>}
      <div style={{ display: "flex", flex: 1, flexDirection: "column", gap: "10px" }}>
       ...
       {!certiDetails &&   <option value="new">New Certificate Template</option>}
        </select>
  
        {!certiDetails && <input
  type="text"
  placeholder="Enter new certificate name"
  value={selectedTemplate.name === 'New Certificate Template' ? newCertificateName : selectedTemplate.name}
...
>
	...
	{!certiDetails && (selectedTemplate.name === 'New Certificate Template'?(<span><button style={{ backgroundColor:"#DDE5BF", color:"#53BB6F" }} onClick={saveTemplate}>Save Template</button></span>): (<span><button style={{ backgroundColor:"#FFF2C6", color:"#FBCB25" }} onClick={updateTemplate}>Update Template</button></span>))}
 {!certiDetails && selectedTemplate.name !== 'New Certificate Template' &&(<span><button style={{ backgroundColor:"#FFE0D3", color:"#FD7238" }} onClick={(e)=>deleteTemplate(selectedTemplate._id)}>Delete Template</button></span>)}
